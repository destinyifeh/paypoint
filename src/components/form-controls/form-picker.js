      ...
      <View style={{flex: 1}}>
        <FormInput 
          ...
          onChangeText={(value) => {
            ...
            setValue(value);
            setChoicesInScope(
              lists.filter(({label}) => label.toLowerCase().includes(value.toLowerCase())).slice(0, MAX_SELECT_BOX_RECORDS)
            )
          }} 
        ...
        <ScrollView contentContainerStyle={{}}>
          {options.map(
            ({label, value}, index) => <ClickableListItem
              disabled={disabledOptions?.includes(value)}
              key={index}
              onPress={() => onSelect(value, index + 1)}
              style={{
                flexDirection: 'row',
                opacity: disabledOptions?.includes(value) ? .25 : 1,
                padding: 16,
              }}
            >
              <Text black>
...
export default class FormPicker extends React.Component {
    ...
    try {
      if (prevProps.defaultValue !== this.props.defaultValue) {
        const isDefaultValueCustom = (
          ...
          !this.props.choices.map(value => value.value).includes(this.props.defaultValue) &&
          this.props.choices?.length
        );
    
        console.log('COMPONENT DID UPDATE', {isDefaultValueCustom}, this.props.choices.map(value => value.value));
  ...
  shouldComponentUpdate(nextProps, nextState) {
    try {
      ...
      if (nextProps.propagateError !== this.props.propagateError && validators.required) {
        (this.state.value === null || this.state.value === this.props.showTextInputForOption) && this.setState({
  ...
  render() {
    const {
      choices, defaultValue, disabled, disabledOptions, hideSearch=false,
      showTextInputForOption, silenceErrors,
    } = this.props;
    const { focused, showOptions, showTextInput, value } = this.state;

    const isDefaultValueCustom = defaultValue && !choices.map(value => value.value).includes(defaultValue);
    const defaultValue_ = isDefaultValueCustom ? showTextInputForOption : defaultValue;
    ...
    const selectedValue_ = showTextInput ? showTextInputForOption : (labelForValue || placeholder);
    ...
    return (
      ...
      >
        ...
        <View style={{
          ...
          >
            <View style={{justifyContent: 'center', height: '100%', padding: 20, width: '100%'}}>
          ...
          {showTextInput && <FormInput
            ...
            onChangeText={(value, fieldIsValid) => {     
              ...
              this.setState({
                errorMessage: fieldIsValid ? null : 'Field must have at least three (3) characters.',
                fieldIsValid,
                value
              });
            }}
