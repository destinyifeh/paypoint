...
import PrintManager from '../utils/print-manager';
...
const {IntentHelper, PaxPrinterHelper} = NativeModules;
...
const printReceiptCapture = async receiptRef => {
  await captureRef(receiptRef, {
    ...
    .then(
      uri => {
        PaxPrinterHelper.printBitmap(uri, 136, 384, this.errorCallback);
      },
      error =>
        PaxPrinterHelper.printStr('Oops, snapshot failed: ' + error, '2'),
    )
...
function Receipt({
  ...
  const checkprintPassword = async () => {
    if (printPassword == newRemoteConfig.print_restriction_password) {
      ...
      printReceiptCapture(receiptView.current);
  ...
  accountOpeningTemplate(allFields) {
    ...
    return (
      <View
        ...
        <View style={{flexDirection: 'row', paddingVertical: 6, marginTop: 6}}>
