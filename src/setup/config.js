...
import {
  ...
  REMOTE_CONFIG_KEYS,
  REMOTE_CONFIG_REFRESH_INTERVAL_IN_SECONDS,
...
async function patchRemoteConfigValues(remoteConfig) {
  ...
  return {
    ...remoteConfig,
    userIsAnInternalAgent: remoteConfig.account_opening_pilot_group.includes(
      userData.username,
    ),
  };
}
...
export async function getRemoteConfig() {
  ...
  REMOTE_CONFIG_KEYS.map(key => {
    if (snapshot[key]) {
      ...
      console.warn(`Key "${key}" is missing from remote config`);
    }
  });
...
setInterval(
  () => getRemoteConfig(),
  REMOTE_CONFIG_REFRESH_INTERVAL_IN_SECONDS * 1000,
);
